...
def get_quotes():
	...
	page_elements = page_div.findAll('a')  # get all the links from the div
	...
	for i in list(pages.keys())[1:]:  # iterating over every page to get the quotes and their links
		...
		for stock in table.findAll('td'):  # iterate over every table data in table on page
			links = stock.findAll('a')  # getting 'a' tag from the table data
